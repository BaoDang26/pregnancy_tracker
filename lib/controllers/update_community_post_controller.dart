...
class UpdateCommunityPostController extends GetxController {
  ...
  // Biến lưu trạng thái loading và lỗi
  ...
  void _initFormValues() {
    ...
    contentController.text = post.value.content ?? '';
    attachmentUrlController.text = post.value.attachmentUrl ?? '';

    // Đánh dấu đã có ảnh nếu attachmentUrl không rỗng
    if (post.value.attachmentUrl != null &&
        post.value.attachmentUrl!.isNotEmpty) {
  ...
  // Cập nhật community post
  Future<bool> updateCommunityPost() async {
    ...
    try {
      ...
      CommunityPostModel postData = CommunityPostModel(
        ...
        attachmentUrl: attachmentUrlController.text,
      );
